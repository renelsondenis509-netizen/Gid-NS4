...
const IcoGraduation = () => (
  <svg width="52" height="52" viewBox="0 0 24 24" fill="none" stroke="#60a5fa" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const IcoTrophy = () => (
  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#fbbf24" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    ...
    <path d="M4 7c0 0-1 5 3 6"/>
    <path d="M20 7c0 0 1 5-3 6"/>
  </svg>
);
const IcoThumbUp = () => (
...
export function ExerciceScreen({ user, scan, onBack, onNavigate }) {
  ...
  function generateLocalQuiz(text, subject) {
    ...
    for (let i = 0; i < sentences.length && qs.length < 5; i++) {
      ...
      const keyWord =
        words.find(w => w[0] === w[0].toUpperCase() && w.length > 4 && !used.has(w)) ||
  ...
  if (loading) return (
    <div className="fixed inset-0 flex flex-col items-center justify-center" style={{ background: "#0a0f2e" }}>
      <div className="flex gap-2 mb-4">
        {[0, 1, 2].map(i => (
          <div key={i} className="w-2.5 h-2.5 rounded-full bg-blue-400" style={{ animation: `bounce 1s ${i * 0.2}s infinite` }} />
        ))}
      </div>
      ...
      <div className="fixed inset-0 flex flex-col items-center justify-center px-6"
        ...
        <div className="w-full max-w-sm space-y-4" style={{ animation: "popIn .5s cubic-bezier(.34,1.56,.64,1) both" }}>
          ...
          <div className="rounded-3xl px-5 py-5 text-center" style={{ background: "rgba(37,99,235,0.15)", border: "2px solid rgba(37,99,235,0.35)" }}>
            ...
            <div className="flex items-center justify-center gap-2 mt-2">
              ...
              <span className="text-white font-bold text-base">
                {isPerfect ? "Pafè !" : isGood ? "Byen !" : "Kontinye travay !"}
              </span>
            </div>
          </div>
          ...
          <div className="space-y-3" style={{ maxHeight: 300, overflowY: "auto" }}>
            {answers.map((a, i) => (
              <div key={i} className="rounded-2xl p-3" style={{
                ...
                <p className="flex items-center gap-1 text-xs" style={{ color: a.correct ? "#86efac" : "#fca5a5" }}>
                  <span style={{ color: a.correct ? "#86efac" : "#fca5a5" }}>
                    {a.correct ? <IcoCheck /> : <IcoX />}
                  </span>
                  {a.choices[a.selected]}
                </p>
                {!a.correct && (
                  <p className="flex items-center gap-1 text-green-400 text-xs mt-0.5">
                    <span style={{ color: "#86efac" }}><IcoCheck /></span>
                    {a.choices[a.answer]}
                  </p>
                )}
                {a.explanation && (
                  <p className="text-blue-400 text-xs mt-1 italic">{a.explanation}</p>
                )}
              </div>
            ))}
          </div>
          ...
          {/* Bouton retour */}
          <button onClick={onBack} className="w-full py-4 rounded-2xl font-bold text-white flex items-center justify-center gap-2"
            style={{ background: "linear-gradient(135deg,#2563eb,#3b82f6)" }}>
            <IcoArrowLeft />
            Retounen nan istorik
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#0a0f2e" }}>
      ...
      <div className="px-4 py-4 border-b flex items-center gap-3" style={{ background: "rgba(10,15,46,0.98)", borderColor: "#ffffff10" }}>
        <button onClick={onBack} className="text-blue-400 flex items-center">
          <IcoArrowLeft />
        </button>
          ...
          {q.choices.map((choice, idx) => {
            ...
            return (
              <button key={idx} onClick={() => handleChoice(idx)}
                ...
                <span className="font-black text-sm w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0"
